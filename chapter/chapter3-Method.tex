...
在BLE Mesh網路中，封包傳輸的效率與穩定性對整體系統效能具有決定性影響。特別是在多跳網路架構中，位於拓樸上層的節點往往需負責更多封包的轉送任務，因此其封包處理與傳輸壓力遠高於下層節點。若未妥善配置這些節點的傳輸參數，將容易造成sent buffer累積、重傳增加與通訊延遲。

為了改善上述問題，本研究透過修改每一層CI連線參數。設計重點在於針對節點在拓樸中的層級位置，分層調整其CI時間，以使越靠近Root的節點，調整成相對較短的時間，讓越接近Root的節點擁有更頻繁的傳輸機會，能有效減緩靠近 Root 節點的 sent buffer 壓力，減少封包重傳次數，提升整體 BLE Mesh 網路的穩定性與可靠性。
