...
從圖 \ref{fig: 實作sink=1初始化拓樸圖} 與圖 \ref{fig: 實作sink=1重啟拓樸圖} 可以觀察到，節點在拓樸中的連線關係雖有變化，但系統仍能保持整體連通性與穩定性。

以初始化拓樸為例，節點 1 為根節點，分別與節點 2、3、6 直接相連，而節點 6 則再向下連接節點 4 與 5，形成二層式樹狀結構。然而在節點 1 重啟之後，新的拓樸雖然仍以節點 1 為根節點，但連線結構有所變化：節點 4 由原先節點 6 的子節點，改為直接連接至節點 1，而節點 3 由直接連線至節點 1，改為由節點 6 所管理。
...
當節點 5 重啟後，系統亦能正確識別其為 Sink 節點，並重新建立拓樸結構。從圖 \ref{fig: 實作sink=5初始化拓樸圖} 與圖 \ref{fig: 實作sink=5重啟拓樸圖} 可見，重啟前後的網路拓樸結構一致，各節點的連線關係保持穩定，節點 3、4、6 皆直接連接至節點 5，節點 2 及節點 1 亦維持在原有層級位置。
...
整體網路包含六個節點，並採用樹狀拓樸設計，各節點之 Connection Interval（CI）依其所處層級進行遞增設置，從 Sink 節點 至 末端節點，分別依序設置 CI = 25 ms、CI = 50 ms、CI = 75 ms 及 CI = 100 ms。
