...
模擬實驗中，分別使用 11、21 及 31 節點進行測試，並針對每種節點數量進行 10 次隨機 Seed 模擬。每次模擬均記錄節點到 Sink 節點的平均 HopsToSink 與最大 HopsToSink 值，並計算11、21 及 31 節點平均結果，以評估拓樸平衡性。
實驗結果如圖 \ref{fig: Avg_hops_comparison} 及 \ref{fig: Max_hops_comparison} 所示，分別列出原生 FruityMesh 與本研究所提出之演算法在不同節點數量下的平均與最大 HopsToSink 值。
從十種 Seed 的平均結果可見，修改版本在各節點數情境中皆展現出更佳的拓樸平衡性。在節點數為 11 的情況下，平均HopsToSink由原生版本的 2.699 降至修改版本的 2.009，最大HopsToSink亦由 4.8 降至 3.8，顯示節點能以更短的路徑連接至 Sink 節點。當節點數增加至 21 時，修改版本同樣表現穩定，平均HopsToSink由 3.524 降至 2.923，最大HopsToSink由 6.5 降為 5.7。即使在節點數量較高的 31 點情境中，修改版本亦可將平均HopsToSink由 4.673 有效降低至 3.561，最大HopsToSink亦由 8.2 降至 6.7。
...
實驗採用與文獻 \cite{112TIT00392032} 相同之實體拓樸架構，如圖 \ref{fig: 實體拓樸架構} 所示。然而，相較於文獻 \cite{112TIT00392032} 於所有節點均採用固定 Connection Interval（CI）設定之方式，本研究進一步提出「分層調整 CI」之機制，依節點於拓樸中所處層級設定不同的連線間隔時間，讓越接近Sink點的節點可以有更多向Sink節點傳輸的時間，提升整體網路穩定性。
整體網路包含六個節點，並採用樹狀拓樸設計，各節點之 Connection Interval（CI）依其所處層級進行遞增設置，從 Sink 節點 至 末端節點，分別依序設置 CI=25、CI=50、CI=75 及 CI=100。
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|}
        ...
        \hline
        Connection Interval & 25 ms（Sink）, 50 ms（1st hop）, 75 ms（2nd hop）, 100 ms（3rd hop） \\
        \hline
        Scan Interval & 25 ms（Sink）, 50 ms（1st hop）, 75 ms（2nd hop）, 100 ms（3rd hop）\\
        \hline
        Connection Event & 10 ms \\
        \hline
...
在連線設定方面，Sink 節點（節點 1）配置最短之 Connection Interval 為 25 ms，而其他節點則依其拓樸中所處層級遞增調整，第一層（1-hop）為 50 ms，第二層為 75 ms，第三層（最遠端節點）為 100 ms，且Scan Interval（SI）時間皆與CI設定值相同。此分層設定的目的在於分散不同節點的通訊時間，可以讓越接近Sink點的節點有夠多傳輸的時間，進而提升封包傳輸的穩定性與效率。

在資料傳輸設定部分，各節點分別傳送 25、50、75 及 100 筆封包，模擬不同負載量下的傳輸表現。每筆封包大小為 30 bytes，封包發送頻率固定為每 100 ms 傳送一筆。為使實驗結果具一致性，所有節點會同步開始傳送資料至 Sink 節點，並於每次連線事件間隔（Connection Event）設為 10 ms 的條件下完成通訊流程。
...
\subsection{平均封包延遲比較}
圖 \ref{fig: avg_delay} 為不同封包發送數量下，各機制所產生之平均封包延遲結果。由圖可觀察到，FruityMesh 原生機制在負載增加時延遲呈現明顯上升趨勢，從 966.07 ms 增加至 1734.08 ms；DOST 機制雖略有改善，但延遲仍達 978.05 ms。而本研究所提出之層級式 CI 調整機制，在四種負載條件下皆能穩定維持低延遲表現，延遲平均僅約為 175 ms 左右，顯示該機制可有效降低傳輸碰撞與佇列等待時間，提升即時性表現。
...
圖 \ref{fig: avg_retrans} 顯示不同機制在各封包數下之平均重傳率變化。原生 FruityMesh 在高負載情境下重傳率明顯偏高，甚至超過 100\%，表示多數封包需多次嘗試方能成功送達，造成網路擁塞與效能下降。DOST 機制的重傳率維持在 14\% 至 19\% 之間，雖低於 FruityMesh，仍顯示網路存在傳輸瓶頸。而本研究所提方法在所有負載下之重傳率皆低於 1.3\%，其中最低僅 0.07\%，顯示其具備優異的穩定性與傳輸效率。
...
圖 \ref{fig: pdr} 比較三種機制在不同封包數量下的封包交付率表現。FruityMesh 的 PDR 隨封包數增加明顯下降，從 99\% 降至 73.08\%，呈現明顯的退化趨勢。DOST 機制與本研究提出之層級式 CI 調整機制則在各情境下皆維持 100\% 的封包交付率，顯示後兩者在處理高負載傳輸時能有效避免封包遺失，維持高度的通訊可靠性。
